...
<%
	...
	if(count>0){
		...
		//				getBoardList(입력한 검색값,시작글번호,한페이지당 보여줄 글 수)
...
<%
	...
	if(count > 0){
		...
		for(int i=0; i<list.size(); i++){
			...
			int beanRe_lev = boardBean.getRe_lev();
			String beanTitle = boardBean.getTitle();
			String beanId = boardBean.getId();
			String beanDate = new SimpleDateFormat("yyyy.MM.dd").format(boardBean.getDate());
			int beanCount = boardBean.getCount();
...
%>
	<tr onclick="location.href='content.jsp?num=<%=beanNum%>&pageNum=<%=pageNum%>'"
		...
		<td class="left">
		<%	
			int wid = 0; // 답변글에 대한 들여쓰기 값 지정
			
			if(boardBean.getRe_lev()>0){ // 답변글에 대한 들여쓰기 값이
				// 들여쓰기 값 처리
